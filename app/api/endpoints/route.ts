...
// GET /api/endpoints — returns live endpoints from gateway_urls with resilience check
export async function GET() {
    // 10-minute window to handle minor network/heartbeat jitter gracefully
    ...
    for (const gw of gateways ?? []) {
        const url = gw.openai_api_url || (gw.tunnel_url ? `${gw.tunnel_url.replace(/\/$/, '')}/v1` : null);
        if (!url) continue;
        if (gw.model === 'pro' && !pro.includes(url)) pro.push(url);
        else if (gw.model === 'ultra' && !ultra.includes(url)) ultra.push(url);
    }
